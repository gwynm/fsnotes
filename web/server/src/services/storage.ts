...
const RESERVED_DIRS = new Set(["i", "files", ".Trash", "Trash", ".git"]);
...
/**
 * Detect the trash directory name: "Trash" (FSNotes default) or ".Trash".
 * Returns whichever exists, preferring "Trash". Creates "Trash" if neither exists.
 */
async function trashDirName(notesRoot: string): Promise<string> {
  try {
    await fs.access(path.join(notesRoot, "Trash"));
    return "Trash";
  } catch {
    // fall through
  }
  try {
    ...
    // Neither exists — create "Trash" (FSNotes convention)
    return "Trash";
  }
}
 ...
 */
export async function ensureDirectories(notesRoot: string): Promise<void> {
  ...
  const trash = await trashDirName(notesRoot);
...
async function collectMdFiles(
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      if (isReservedDir(topDir)) continue;
...
export async function listNotes(
  ...
  const trashName = await trashDirName(notesRoot);

  let searchDir: string;
  if (trash) {
    searchDir = path.join(notesRoot, trashName);
  } else if (folder) {
  ...
  for (const rel of files) {
    try {
      // For trash, the relative path should include the trash dir name
      const actualRel = trash ? path.join(trashName, rel) : rel;
 ...
 */
async function collectMdFilesFlat(dir: string): Promise<string[]> {
  const results: string[] = [];
  let entries;
  try {
    entries = await fs.readdir(dir, { withFileTypes: true });
...
export async function patchNote(
  ...
  try {
    ...
    throw Object.assign(
      new Error(`Note already exists: ${newRelativePath}`),
      { statusCode: 409, code: "ALREADY_EXISTS" }
    );
  } catch (err: unknown) {
    if (
      err &&
 ...
 */
export async function listFolders(notesRoot: string): Promise<Folder[]> {
  const entries = await fs.readdir(notesRoot, { withFileTypes: true });
  const folders: Folder[] = [];

  for (const entry of entries) {
    if (!entry.isDirectory()) continue;
    if (isReservedDir(entry.name)) continue;
 ...
 */
export async function getAllNotePaths(notesRoot: string): Promise<string[]> {
